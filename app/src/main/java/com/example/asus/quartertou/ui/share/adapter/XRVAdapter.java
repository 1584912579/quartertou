package com.example.asus.quartertou.ui.share.adapter;
...
public class XRVAdapter extends RecyclerView.Adapter {
    ...
    public XRVAdapter(Context context, List<VideosBean.DataBean> list) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, final int position) {
       ...
       // holder1.video.setImageURI(dataBean.getUser().getIcon());
        holder1.video.setUp(dataBean.getVideoUrl(),JZVideoPlayerStandard.SCREEN_WINDOW_NORMAL,"谁还不是小仙女呢！");

        Glide.with(context).load(dataBean.getUser().getIcon()).into(holder1.video.thumbImageView);
        ...
        holder1.r.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public void shuxin(List<VideosBean.DataBean> templist){
        this.list.clear();
        this.list.addAll(templist);
        notifyDataSetChanged();

    }
    public void jiazai(List<VideosBean.DataBean> list){
